...
### PaperDiaryUpload Supabase 직접 쿼리 (제거 대상)
위치: `MoodLog/components/PaperDiaryUpload.tsx` lines 47-63
- 기존 entry 날짜 조회 → `GET /entries/dates` 엔드포인트로 교체

### Header 아바타 (수정 대상)
위치: `MoodLog/components/Header.tsx` lines 23-34
- `supabase.auth.getUser()` → `GET /auth/me` 또는 쿠키에서 파싱

### 이미지 최적화 (유지)
위치: `MoodLog/components/PaperDiaryUpload.tsx` lines 189-230
- Canvas 기반 최적화, max 1200px, JPEG 0.85 품질
- 이 로직 유지, 업로드 대상만 S3 presigned URL로 변경
